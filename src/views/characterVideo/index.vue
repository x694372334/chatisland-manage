    ...
    <div>
      <el-row>
        <draggable v-model="sortList" @end="onEnd">
          <el-col v-for="item in videoList" :span="8" style="margin: 5px 0;">
            <el-card style="height: 600px;">
              <div slot="header" class="clearfix">
                <el-col :span="18">
                  <span style="font-size: 12px;">{{item.videoFileName}}</span>
                </el-col>
                <el-col :span="6" style="text-align: right;">
                  <el-button type="danger" size="mini" @click="handleDelete(item.id, item.videoFileName)">删除</el-button>
                </el-col>
              </div>
<!--              <el-image style="max-width: 100%;" :src="item.videoFilePath"></el-image>-->
              <video style="max-width: 100%;" :src="item.videoFilePath"></video>
            </el-card>
          </el-col>
        </draggable>
...
<!--    <el-table v-loading="loading" :data="postList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="文件预览" align="center">
        <template slot-scope="scope">
          <img :src="scope.row.videoFilePath" style="width:240px;height: 180px;">
        </template>
      </el-table-column>
      <el-table-column label="文件名称" align="center" prop="videoFileName"/>
...
<script>
import {selectAllSystemUserList, listVideo, uploadVideos, delVideo, saveVideoSort} from "@/api/chatislandApi/characterVideo";
...
export default {
  ...
  methods: {
    ...
    handleSort(){
      ...
      saveVideoSort(list).then(response => {
        if(response.data){
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          if (this.form.userId != null) {
            ...
            uploadVideos(formData).then(response => {
